...
#include "../source/core/mk7titles.h"
...
static mk7_ctx_t CTX;

static void ctx_setup(void)
{
    mk7_ctx_init(&CTX, ROOT, TID);
    mk7_ensure_layout(&CTX);
}
...
{
    printf("titles\n");
    check(mk7_lookup(0x0004000000030800ULL) != NULL, "USA id known");
    check(mk7_lookup(0x0004000000030700ULL) != NULL, "EUR id known");
    check(mk7_lookup(0x0004000000030600ULL) != NULL, "JPN id known");
    check(mk7_lookup(0x0004000000030A00ULL) != NULL, "KOR id known");
    check(mk7_lookup(0x0004000000123456ULL) == NULL, "unknown id rejected");
    check(mk7_lookup_hex("0004000000030800") != NULL, "hex lookup");
    check(mk7_lookup_hex("0004000000030a00") != NULL, "hex lookup is case-insensitive");
    check(mk7_lookup_hex("nonsense") == NULL, "bad hex rejected");
}
...
static void t_ctx_init(void)
{
    printf("ctx init\n");
    mk7_ctx_t c;
    check(mk7_ctx_init(&c, "./x", TID), "init accepts valid args");
    check_eq_str(c.root, "./x/", "root gains a trailing slash");

    check(mk7_ctx_init(&c, "./x/", "0004000000030a00"), "init accepts lowercase hex");
    check_eq_str(c.title_hex, "0004000000030A00", "hex is uppercased");
    check_eq_str(c.root, "./x/", "existing trailing slash not doubled");

    check(!mk7_ctx_init(&c, "./x", "TOOSHORT"), "short title id rejected");
    check(!mk7_ctx_init(&c, "", TID), "empty root rejected");

    char longroot[MK7_MAX_ROOT + 8];
    memset(longroot, 'a', sizeof(longroot) - 1);
    longroot[sizeof(longroot) - 1] = 0;
    check(!mk7_ctx_init(&c, longroot, TID), "over-long root rejected, not truncated");
}
...
static void t_long_slug_skipped(void)
{
    ...
    char longslug[MK7_MAX_SLUG + 10];
    memset(longslug, 'z', sizeof(longslug) - 1);
    longslug[sizeof(longslug) - 1] = 0;

    char rel[PLAT_MAX_PATH];
    snprintf(rel, sizeof(rel), "mk7mods/%s/layeredfs/a.szs", longslug);
    mkfile(rel, "TOO-LONG");
...
static void t_layout(void)
{
    ...
    sd_reset();
    ctx_setup();
    check(rel_exists("mk7mods"), "mk7mods created");
    check(rel_exists("luma/titles"), "luma/titles created");
    check(rel_exists("luma/plugins"), "luma/plugins created");

    check(mk7_ensure_layout(&CTX) == SWAP_OK, "ensure_layout is repeatable");
}
...
static void t_adopt_empty(void)
{
    ...
    sd_reset();
    ctx_setup();
    check(mk7_adopt_if_needed(&CTX) == SWAP_OK, "adopt succeeds");

    char active[MK7_MAX_SLUG];
    mk7_read_active(&CTX, active, sizeof(active));
    check_eq_str(active, MK7_SLUG_STOCK, "clean card adopts as stock");
}
...
static void t_adopt_existing_mod(void)
{
    ...
    char active[MK7_MAX_SLUG];
    mk7_read_active(&CTX, active, sizeof(active));
    check_eq_str(active, MK7_SLUG_EXISTING, "pre-existing mod adopted as 'existing'");
...
static void t_adopt_ctgp(void)
{
    ...
    char active[MK7_MAX_SLUG];
    mk7_read_active(&CTX, active, sizeof(active));
    check_eq_str(active, MK7_SLUG_CTGP, "live plugin + CTGP folder adopts as ctgp7");
...
static void t_adopt_is_once_only(void)
{
    ...
    ctx_setup();
    mk7_adopt_if_needed(&CTX);
    mk7_swap_to(&CTX, MK7_SLUG_STOCK);
    ...
    char active[MK7_MAX_SLUG];
    mk7_read_active(&CTX, active, sizeof(active));
    check_eq_str(active, MK7_SLUG_STOCK, "adopt is a no-op once state exists");
}
...
static void t_swap_basic(void)
{
    ...
    check(mk7_swap_to(&CTX, "custom") == SWAP_OK, "swap to custom");
    check(rel_has("luma/titles/" TID "/romfs/course/mirror.szs", "CUSTOM-TRACK"),
          "custom track is live");
    check(!rel_exists("mk7mods/custom/layeredfs"), "parked copy moved out");

    char active[MK7_MAX_SLUG];
    mk7_read_active(&CTX, active, sizeof(active));
    check_eq_str(active, "custom", "state records custom");

    check(mk7_swap_to(&CTX, MK7_SLUG_STOCK) == SWAP_OK, "swap to stock");
    check(rel_has("mk7mods/custom/layeredfs/romfs/course/mirror.szs", "CUSTOM-TRACK"),
          "custom track parked again, intact");
    check(count_content(ROOT "luma", "CUSTOM-TRACK") == 0, "nothing custom left live");

    mk7_read_active(&CTX, active, sizeof(active));
    check_eq_str(active, MK7_SLUG_STOCK, "state records stock");
}
...
static void t_swap_idempotent(void)
{
    ...
    sd_reset();
    mkfile("mk7mods/custom/layeredfs/romfs/a.szs", "CUSTOM-TRACK");
    ctx_setup();
    mk7_adopt_if_needed(&CTX);

    check(mk7_swap_to(&CTX, "custom") == SWAP_OK, "first swap");
    check(mk7_swap_to(&CTX, "custom") == SWAP_OK, "second swap to the same mod");
    check(mk7_swap_to(&CTX, "custom") == SWAP_OK, "third swap to the same mod");
...
static void t_swap_ctgp_cycle(void)
{
    ...
    check(mk7_swap_to(&CTX, "custom") == SWAP_OK, "CTGP -> custom");
    check(count_content(ROOT "luma/plugins", "CTGP-PLUGIN") == 0,
          "CTGP plugin is completely gone from the live slot");
    check(rel_has("mk7mods/ctgp7/plugins/CTGP-7.3gx", "CTGP-PLUGIN"), "CTGP plugin parked intact");
    check(rel_has("luma/titles/" TID "/romfs/course/mirror.szs", "CUSTOM-TRACK"), "custom live");

    check(mk7_swap_to(&CTX, MK7_SLUG_STOCK) == SWAP_OK, "custom -> stock");
    ...
    check(mk7_swap_to(&CTX, MK7_SLUG_CTGP) == SWAP_OK, "stock -> CTGP");
...
static void t_conflict_refused(void)
{
    ...
    sd_reset();
    mkfile("mk7mods/custom/layeredfs/a.szs", "PARKED-COPY");
    ...
    check(rel_has("mk7mods/custom/layeredfs/a.szs", "PARKED-COPY"), "parked copy survived");
...
static void t_empty_dir_not_blocking(void)
{
    ...
    check(mk7_swap_to(&CTX, "custom") == SWAP_OK, "swap succeeds past the empty folder");
...
static void t_list_and_detect(void)
{
    ...
    for (int i = 0; i < n; i++) {
        if (!strcmp(mods[i].slug, MK7_SLUG_CTGP)) {
            saw_ctgp = true;
    ...
    check(mk7_ctgp_available(&CTX), "CTGP detected when folder + plugin present");

    // Parked rather than live must still count as installed.
    mk7_swap_to(&CTX, "custom");
    check(mk7_ctgp_available(&CTX), "CTGP still detected while parked");

    // Folder removed (uninstalled) -> must grey out.
    rm_rf(ROOT "CTGP-7");
    check(!mk7_ctgp_available(&CTX), "CTGP not detected after its folder is removed");
}
...
static void t_detect_no_ctgp(void)
{
    ...
    ctx_setup();
    mk7_adopt_if_needed(&CTX);
    check(!mk7_ctgp_available(&CTX), "CTGP absent on a card that never had it");

    // Leftover folder with no plugin must not count as installed.
    mkfile("CTGP-7/readme.txt", "leftover");
    check(!mk7_ctgp_available(&CTX), "empty CTGP leftover folder does not count");
}
...
static void t_crash_recovery_sweep(void)
{
    ...
    for (int fail_at = 0; fail_at < 6; fail_at++) {
        ...
        mk7_swap_to(&CTX, "custom");   // may fail part-way, that is the point
        plat_host_no_faults();

        mk7_result_t rc = mk7_recover(&CTX);
        ...
        check(!rel_exists("mk7mods/mk7swap.journal"), label);
    }
}
...
static void t_crash_recovery_reverse(void)
{
    ...
    for (int fail_at = 0; fail_at < 6; fail_at++) {
        fixture_ctgp_active_custom_parked();
        mk7_swap_to(&CTX, "custom");   // clean, so we start from custom-active

        plat_host_fail_after(fail_at);
        mk7_swap_to(&CTX, MK7_SLUG_CTGP);
        plat_host_no_faults();
...
static void t_recover_noop(void)
{
    ...
    fixture_ctgp_active_custom_parked();
    check(mk7_recover(&CTX) == SWAP_OK, "recover with no journal is a no-op");
...
static void t_mod_with_both_slots(void)
{
    ...
    sd_reset();
    mkfile("mk7mods/big/layeredfs/romfs/a.szs", "BIG-FS");
    mkfile("mk7mods/big/plugins/big.3gx", "BIG-PLUGIN");
    ctx_setup();
    mk7_adopt_if_needed(&CTX);

    check(mk7_swap_to(&CTX, "big") == SWAP_OK, "swap to a two-slot mod");
    ...
    check(mk7_swap_to(&CTX, MK7_SLUG_STOCK) == SWAP_OK, "back to stock");
...
static void t_journal_points_at_missing_mod(void)
{
    ...
    check(!rel_exists("mk7mods/mk7swap.journal"), "journal cleared");

    // Whatever it decided, the app must still be able to get back to a real mod.
    check(mk7_swap_to(&CTX, MK7_SLUG_CTGP) == SWAP_OK, "can still swap to CTGP after");
    check(rel_has("luma/plugins/" TID "/CTGP-7.3gx", "CTGP-PLUGIN"), "CTGP live again");
}
...
static void t_target_deleted_between_boots(void)
{
    ...
    check(mk7_swap_to(&CTX, MK7_SLUG_STOCK) == SWAP_OK, "swap to stock survives it");
    char active[MK7_MAX_SLUG];
    mk7_read_active(&CTX, active, sizeof(active));
    check_eq_str(active, MK7_SLUG_STOCK, "state is stock");
    check(mk7_swap_to(&CTX, "custom") == SWAP_OK, "swapping back does not error");
}
...
static void t_repeated_swaps_no_recover(void)
{
    ...
    for (int i = 0; i < 12; i++) {
        const char *t = (i % 3 == 0) ? MK7_SLUG_CTGP
                      : (i % 3 == 1) ? "custom"
                                     : MK7_SLUG_STOCK;
        if (mk7_swap_to(&CTX, t) != SWAP_OK) {
    ...
    check(!rel_exists("mk7mods/mk7swap.journal"), "no journal left behind");
}
...
static void t_conflict_then_recovers_after_fix(void)
{
    ...
    check(mk7_swap_to(&CTX, MK7_SLUG_STOCK) == SWAP_ERR_CONFLICT, "blocked while ambiguous");

    // The user resolves it by deleting the duplicate parked copy.
    rm_rf(ROOT "mk7mods/custom/layeredfs");

    check(mk7_swap_to(&CTX, MK7_SLUG_STOCK) == SWAP_OK, "works once resolved");
    check(rel_has("mk7mods/custom/layeredfs/a.szs", "LIVE-COPY"), "the live copy was parked");
    check(!rel_exists("mk7mods/mk7swap.journal"), "journal cleared after success");
}
...
static void t_existing_then_cycle(void)
{
    ...
    mkfile("mk7mods/custom/layeredfs/romfs/mine.szs", "CUSTOM-TRACK");
    ctx_setup();
    mk7_adopt_if_needed(&CTX);

    check(mk7_swap_to(&CTX, "custom") == SWAP_OK, "swap away from the adopted mod");
    check(rel_has("mk7mods/existing/layeredfs/romfs/theirs.szs", "SOMEONE-ELSES-MOD"),
          "their mod parked intact, not destroyed");

    check(mk7_swap_to(&CTX, MK7_SLUG_EXISTING) == SWAP_OK, "swap back to theirs");
...
static void t_state_file_corrupt(void)
{
    ...
    fixture_ctgp_active_custom_parked();
    mkfile("mk7mods/mk7swap.state", "\x01\x02 garbage no equals sign\n");

    // Must not crash, must not lose data, must still be usable.
    mk7_result_t rc = mk7_swap_to(&CTX, "custom");
...
static void t_adopt_ctgp_alongside_own_mod(void)
{
    ...
    mk7_ctx_t c;
    mk7_ctx_init(&c, ROOT, TID);
    mk7_ensure_layout(&c);
    mk7_adopt_if_needed(&c);

    char owner[MK7_MAX_SLUG];
    mk7_read_slot_owner(&c, MK7_SLOT_PLUGINS, owner, sizeof(owner));
    check_eq_str(owner, MK7_SLUG_CTGP, "plugin slot adopted as ctgp7");
    mk7_read_slot_owner(&c, MK7_SLOT_LAYEREDFS, owner, sizeof(owner));
    check_eq_str(owner, MK7_SLUG_EXISTING, "layeredfs slot adopted as existing");

    check(mk7_ctgp_available(&c), "CTGP-7 button lit when CTGP-7 is installed");
    ...
    check(mk7_swap_to(&c, MK7_SLUG_CTGP) == SWAP_OK, "swap to ctgp7 succeeds");
    ...
    check(rel_has("mk7mods/existing/layeredfs/romfs/theirs.szs", "THEIR-MOD"),
          "their mod parked under existing");
    check(count_content(ROOT, "THEIR-MOD") == 1, "exactly one copy of their mod");

    // And back again.
    check(mk7_swap_to(&c, MK7_SLUG_EXISTING) == SWAP_OK, "swap back to existing");
    check(rel_has("luma/titles/" TID "/romfs/theirs.szs", "THEIR-MOD"), "their mod live again");
    check(rel_has("mk7mods/ctgp7/plugins/CTGP-7.3gx", "CTGP-PLUGIN"), "CTGP plugin parked");
    check(count_content(ROOT, "CTGP-PLUGIN") == 1, "exactly one copy of the plugin");
    check(mk7_ctgp_available(&c), "CTGP-7 still offered after switching away");
}
